...
public interface Belt {
	...
	/* Register a customer to a place on the belt
	 * Throw a runtime exception if the position is already registered 
	 * Throw a runtime exception if the specified `Customer` object is 
	 * already registered at a different position
	 * Throw an `IllegalArgumentExceptionif theCustomer` object provided is null
	 */
	public void registerCustomerAtPosition(Customer c, int position);
	
	
	/* Unregister the `Customer` object registered at the specified position
	 * and return a reference to that `Customer`.
	 * Returns null if there is no `Customer` registered at that position
	 */
